...
import { CalendarDays, Check, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, LayoutGrid, PanelBottom, PanelLeft, PanelRight, PanelTop, Plus, Redo2, Search, Settings, Undo2, LayoutTemplate, Moon, Package, Palette, Sun, Terminal, Columns2, Rows2, Activity, BookOpen, Folder, File, X, } from "lucide-react";
...
export function Icon({ className, color, name, size = 18, strokeWidth = 1.8 }) {
    const theme = useTheme();
    const Component = iconMap[name];
    return (_jsx(Component, { className: className, color: color ?? theme.colors.foreground, size: size, strokeWidth: strokeWidth }));
}
