...
export function Combobox({ children, className, defaultOpen, defaultQuery, defaultValue, onOpenChange, onQueryChange, onValueChange, open, placeholder, query, value, ...props }) {
...
export const ComboboxValue = React.forwardRef(({ className, children, ...props }, ref) => {
    const context = useComboboxContext();
...
export const ComboboxContent = React.forwardRef(({ className, ...props }, ref) => {
    const context = useComboboxContext();
    if (!context.open) {
        return null;
    }
    return (_jsx("div", { ref: ref, className: cn("absolute left-0 right-0 top-[calc(100%+0.5rem)] z-50 rounded-[calc(var(--sx-radius-lg)+2px)] border border-[color:var(--sx-color-border)] bg-[color:var(--sx-color-surface)] shadow-[0_18px_42px_rgba(15,23,42,0.14)]", className), ...props }));
});
ComboboxContent.displayName = "ComboboxContent";
export const ComboboxInput = React.forwardRef(({ className, onChange, placeholder = "Search options", type = "text", ...props }, ref) => {
    const context = useComboboxContext();
...
export const ComboboxList = React.forwardRef(({ className, ...props }, ref) => (_jsx("div", { ref: ref, role: "listbox", className: cn("max-h-64 overflow-y-auto p-2", className), ...props })));
ComboboxList.displayName = "ComboboxList";
export const ComboboxEmpty = React.forwardRef(({ children = "No results found.", className, ...props }, ref) => {
    const context = useComboboxContext();
...
export const ComboboxItem = React.forwardRef(({ children, className, keywords, onClick, textValue, value, ...props }, ref) => {
    const context = useComboboxContext();
