...
import { Inline, ScrollArea, Stack, Surface, type SurfaceProps } from "../primitives/index.native";
...
export function AppShell({ children, header, sidebar }: AppShellProps) {
  return (
    <Stack grow background="background" foreground="foreground" style={{ flex: 1 }}>
      {header ? <Surface tone="raised" border={false} padding="md">{header}</Surface> : null}
      <Inline grow style={{ flex: 1 }}>
        {sidebar ? (
          <Surface
            tone="muted"
            border={false}
        ...
        <ScrollArea grow contentContainerStyle={{ minHeight: "100%" }} style={{ flex: 1 }}>
          {children}
        </ScrollArea>
      </Inline>
    </Stack>
