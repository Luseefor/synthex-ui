...
export function resolveSpace(theme: SynthexTheme, value?: SpaceValue): number | undefined {
  if (value === undefined) {
    return undefined;
  }
...
export function resolveRadius(theme: SynthexTheme, value?: RadiusValue): number | undefined {
  if (value === undefined) {
    return undefined;
  }
...
export function resolveColor(theme: SynthexTheme, value?: ColorValue): string | undefined {
  if (!value) {
    return undefined;
  }

  if (value in theme.colors) {
    return theme.colors[value as keyof ColorScale];
  }

  return value;
}

export function resolveShadow(theme: SynthexTheme, value?: ShadowValue): string | undefined {
  if (!value) {
    return undefined;
  }

  if (value in theme.shadows) {
    return theme.shadows[value as keyof ShadowScale];
  }

  return value;
}

export function createBoxStyle(theme: SynthexTheme, props: BoxStyleProps) {
  ...
  return {
    ...
    color,
    display:
      props.direction || props.gap !== undefined || props.align || props.justify || props.wrap
        ? "flex"
        : undefined,
    ...
    flexShrink:
      props.shrink === true ? 1 : typeof props.shrink === "number" ? props.shrink : undefined,
...
export function createSurfaceStyle(theme: SynthexTheme, props: SurfaceStyleProps) {
  ...
  return {
    ...
    borderColor:
      tone === "accent" ? theme.colors.accent : tone === "transparent" ? "transparent" : theme.colors.border,
  } as const;
}

export function createTextStyle(theme: SynthexTheme, props: TextStyleProps) {
